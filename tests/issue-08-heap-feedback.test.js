...
function buildDom(stoneCount) {
  ...
  heapEl.querySelectorAll = function (selector) {
    if (selector === ".stone") return heapEl._children.filter((c) => !c.removed);
    return [];
  };
  heapEl.querySelector = function (selector) {
    if (selector === ".heap-feedback") {
      return heapEl._children.find((c) => c.className === "heap-feedback") || null;
    }
    return null;
  };
...
function run(NimImpl) {
  ...
  const window = {
    Game: {},
    Nim: NimImpl,
    AI: { chooseMove() { assert.fail("KI must not fire here"); } }
  };
  const firedTimers = [];
  const context = {
    window,
    ...
    Uint32Array,
    parseInt,
    setTimeout(fn) { firedTimers.push(fn); return {}; },
    clearTimeout() {}
  };
  vm.runInNewContext(
...
(function case1() {
  ...
  reset(Game, [7], nim);
  Game.commitTap(0, 5);
  assert.deepStrictEqual(Game.state.heaps, [7], "5 > 4 (4er-Regel) must NOT move");
  ...
  assert.ok(/4/.test(text) && /nur/i.test(text),
  ...
  assert.strictEqual(Game.state.lock, false, "no lock may remain after the feedback");
  Game.commitTap(0, 3);
  assert.deepStrictEqual(Game.state.heaps, [4], "the next legal tap must move immediately");
  assert.strictEqual(Game.state.lastMove.amount, 3, "the legal move must commit");
})();
...
(function case2() {
  ...
  assert.strictEqual(Game.state.lastMove, null, "no move must be recorded");
  assert.strictEqual(Game.state.undoStack.length, 0, "no undo snapshot must be pushed");
  const text = feedbackText(heapEl);
  assert.ok(text, "a heap feedback note must appear");
  assert.ok(/1/.test(text) && /3/.test(text) && /5/.test(text),
    "the feedback must list the allowed numbers 1, 3, 5, got: " + text);
  Game.commitTap(0, 5);
  assert.deepStrictEqual(Game.state.heaps, [2], "a listed amount (5) must move");
})();
...
(function case3() {
  ...
  reset(Game, [3], nim);
  Game.commitTap(0, 9);
  assert.deepStrictEqual(Game.state.heaps, [3], "an oversized amount must NOT move");
  assert.strictEqual(Game.state.lastMove, null, "no move must be recorded");
  const text = feedbackText(heapEl);
  assert.ok(text, "a heap feedback note must appear");
  assert.ok(/nicht da/i.test(text),
    "the feedback must say the amount is not there, got: " + text);
  Game.commitTap(0, 2);
  assert.deepStrictEqual(Game.state.heaps, [1], "a legal amount must still move");
})();
