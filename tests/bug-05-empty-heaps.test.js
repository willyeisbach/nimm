...
function makeElement() {
    ...
    querySelectorAll(selector) {
      if (selector === ".stone") {
        return this.children.filter((child) =>
          child.classList.contains("stone"),
        );
      }
      return [];
    },
    closest(selector) {
      return selector === ".heap" && this.classList.contains("heap")
        ? this
        : null;
    },
    focus() {},
  ...
  Object.defineProperty(element, "className", {
    get() {
      return Array.from(classList.classes || []).join(" ");
    },
    set(value) {
      classList.setFromClassName(String(value));
    },
  });
  return element;
}
...
assert.deepStrictEqual(
  Game.state.heaps,
  [0, 3, 2],
  "the empty-heap state must be reproducible",
);
assert.strictEqual(
  Game.state.selectedHeap,
  1,
  "start must select the first non-empty heap",
);
...
const stonesWrap = emptyHeap.children.filter((child) =>
  child.classList.contains("heap-stones"),
);
assert.strictEqual(
  stonesWrap.length,
  1,
  "an empty heap must keep its .heap-stones area",
);
const emptyNote = stonesWrap[0].children.filter((child) =>
  child.classList.contains("empty-note"),
);
assert.strictEqual(
  emptyNote.length,
  1,
  "an empty heap must show exactly one 'leer'-Notiz",
);
assert.ok(
...
assert.strictEqual(
  stonesWrap[0].children.filter((child) => child.classList.contains("stone"))
    .length,
  0,
...
const legalStonesWrap = legalHeap.children.filter((child) =>
  child.classList.contains("heap-stones"),
);
assert.strictEqual(
  legalStonesWrap[0].children.filter((child) =>
    child.classList.contains("stone"),
  ).length,
  3,
...
assert.strictEqual(
  Game.state.selectedHeap,
  1,
  "render must not leave an empty heap selected",
);
assert.strictEqual(
  emptyHeap.tabIndex,
  -1,
  "an empty heap must not be focusable",
);
assert.strictEqual(
  emptyHeap.getAttribute("role"),
  "none",
  "an empty heap must not expose an interactive role",
);
assert.strictEqual(
  emptyHeap.getAttribute("aria-pressed"),
  null,
  "an empty role=none heap must not expose aria-pressed",
);
assert.strictEqual(
  legalHeap.tabIndex,
  0,
  "a non-empty heap must remain focusable",
);
...
assert.strictEqual(
  Game.state.selectedHeap,
  1,
  "clicking an empty heap must be ignored",
);
...
assert.strictEqual(
  Game.state.selectedHeap,
  1,
  "keyboard activation of an empty heap must be ignored",
);
assert.strictEqual(legalHeap.getAttribute("aria-pressed"), "true");
assert.strictEqual(
  emptyHeap.getAttribute("aria-pressed"),
  null,
  "an empty role=none heap must remain without aria-pressed",
);
...
assert.strictEqual(
  Game.state.selectedHeap,
  2,
  "another non-empty heap must remain selectable immediately",
);
...
assert.deepStrictEqual(
  Game.state.heaps,
  [1],
  "an entirely empty generation must be replaced by a playable heap",
);
assert.ok(
  Game.state.heaps.some((size) => size >= 1),
  "a minSteine=0 start must contain a stone",
);
assert.ok(
  Game.state.heaps.every((size) => size >= 0 && size <= 3),
  "the fallback must stay within the configured bounds",
);
assert.strictEqual(
  Game.state.selectedHeap,
  0,
  "the fallback heap must be selected",
);
