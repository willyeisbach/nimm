...
assert.ok(indexHtml.indexOf('id="rule-hint"') !== -1,
...
function makeClassList() {
  ...
  return {
    ...
    toggle(name) { return classes.has(name); },
...
const document = {
  ...
  querySelector(selector) {
    if (selector === "#rule-hint") { return ruleHintEl; }
    if (selector === "#heaps") { return generic; }
...
function loadGame() {
  const window = {
    ...
    Nim: {
      parseAllowed(rule, ownList) {
        if (rule === "classic") { return null; }
        if (rule === "4er") { return [1, 2, 3, 4]; }
        // ownList: kommagetrennte Zahlen → Array
        return String(ownList).split(",").map(function (x) { return parseInt(x, 10); })
  ...
  context.window.crypto = {
    getRandomValues(buf) { for (let i = 0; i < buf.length; i++) { buf[i] = 42; } return buf; }
  };
...
(function ak1() {
  ...
  assert.ok(/1,\s*2,\s*3\s+oder\s+4/.test(text),
    "4er-Regel: 1,2,3 oder 4 genannt. Got: " + text);
  assert.ok(/einem Haufen/.test(text), "«nur aus einem Haufen» muss enthalten sein. Got: " + text);
  assert.ok(/letzte nimmt, gewinnt/i.test(text), "«letzte gewinnt» muss enthalten sein. Got: " + text);
  assert.ok(/Grundy|Mod-4|Bitparität|NIM/.test(text) === false,
...
(function ak2() {
  ...
  const text = ruleHintEl.textContent;
  assert.ok(/so viele/.test(text), "Klassisch: 'so viele' muss vorkommen. Got: " + text);
  assert.ok(/1,\s*2,\s*3\s+oder\s+4/.test(text) === false,
    "Klassisch soll nicht die 4er-Liste nennen. Got: " + text);
  assert.ok(/einem Haufen/.test(text), "«nur aus einem Haufen» muss enthalten sein.");
})();
...
(function ak3() {
  ...
  const text = ruleHintEl.textContent;
  assert.ok(/1,\s*3\s+oder\s+5/.test(text),
...
(function akSwitch() {
  ...
  assert.ok(/1,\s*3\s+oder\s*5/.test(before), "Vorher: eigene Liste. Got: " + before);
  ...
  assert.ok(/so viele/.test(after), "Nach Wechsel: Klassisch-Text. Got: " + after);
  assert.ok(/1,\s*3\s+oder\s*5/.test(after) === false,
    "Nach Wechsel: die alten Listenzahlen dürfen nicht mehr stehen. Got: " + after);
})();

console.log("Issue 02 regression test passed (rule hint on the board, rule-dependent)");
