...
function makeElement() {
  ...
  return {
    ...
    classList: {
      ...
      toggle(name, force) {
        if (force === undefined ? !classes.has(name) : force) { classes.add(name); return true; }
        classes.delete(name);
        return false;
      },
...
const document = {
  ...
  querySelectorAll(selector) {
    if (selector === "#heaps .heap") { return [heap]; }
    if (selector === ".char-card") { return [charWrap]; }
    return [];
  },
...
const window = {
  Game: {},
  crypto: { getRandomValues(buffer) { buffer[0] = 0x1234; return buffer; } },
  Nim: {
    parseAllowed() { return null; },
    legalAmount(rule, allowed, amount, heapSize) { return amount >= 1 && amount <= heapSize; },
    nimSum(position) { return position.reduce((s, n) => s ^ n, 0); }
  },
  AI: { chooseMove() { return { heapIdx: 0, amount: 1 }; } }
};
const context = {
  window, document, console, Math, Number, Array, Uint32Array, parseInt,
...
assert.deepStrictEqual(Game.undoStack().length, 1, "one snapshot must be pushed");
...
assert.strictEqual(Game.state.active, 1, "undo must restore the player who moved");
assert.strictEqual(undoBtn.disabled, true, "undo must be disabled when stack is empty");
...
Game.state.active = 2;
Game.undoMove();
assert.deepStrictEqual(Game.state.heaps, [4], "AI undo must roll back AI + human move");
assert.strictEqual(Game.state.active, 1, "AI undo must hand back to the human");
assert.strictEqual(Game.undoStack().length, 0, "AI undo must drain the whole stack");
assert.strictEqual(undoBtn.disabled, true, "undo must disable after stack is drained");
...
Game.state.active = 1;
Game.pushHistory();
assert.strictEqual(undoBtn.disabled, true, "undo must stay disabled when the option is off");
Game.undoMove(); // muss no-op sein
assert.deepStrictEqual(Game.state.heaps, [2], "undo must not move when disabled");
assert.strictEqual(Game.undoStack().length, 1, "the snapshot must stay on the stack");

// Undo wird während Lock ignoriert.
Game.state.undoEnabled = true;
Game.state.lock = true;
Game.undoMove();
assert.deepStrictEqual(Game.state.heaps, [2], "undo must be ignored while locked");
Game.state.lock = false;
