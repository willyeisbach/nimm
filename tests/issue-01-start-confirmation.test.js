...
startOverlay.querySelector = function (sel) {
  if (sel === "#start-who") { return startWho; }
  return null;
};
...
const document = {
  ...
  querySelector(selector) {
    if (selector === "#start-overlay") { return startOverlay; }
    return element();
  },
...
function freshSetup(Game, opponent) {
  ...
  Game.newHeaps = function () { return [6, 4]; };
  s.maxHaufen = 2;
  s.rule = "4er";
  s.maxSteine = 10;
  s.minSteine = 4;
  ...
  return {
    aiCalls: function () { return aiCalls; }
  };
}
...
(function ak1ai() {
  ...
  assert.deepStrictEqual(s.heaps, [6, 4], "AK1: keine Rosine entfernt vor Los!");
  assert.strictEqual(hooks.aiCalls(), 0, "AK6: kein maybeAIMove vor Bestätigung");
  assert.ok(!startOverlay.hidden, "AK1: Start-Panel sichtbar");
  assert.ok(/Baxi beginnt/.test(startWho.textContent),
...
(function ak3human() {
  ...
  Game.executeMove();
  assert.deepStrictEqual(s.heaps, [6, 4], "AK4: executeMove vor Los! blockiert");
  Game.confirmStart();
  // Nach Los! darf der Mensch sofort ziehen.
  Game.commitTap(0, 2);
  assert.deepStrictEqual(s.heaps, [4, 4], "AK3: nach Los! kann sofort gezogen werden (6→4)");
  assert.strictEqual(s.active, 2, "Zugwechsel zu Spieler 2");
  // MvM: nur der Mensch hat gezogen (Zug 1, Spieler 1) — die KI ist still.
  assert.strictEqual(s.lastMove.player, 1, "AK3: MvM — der Zug gehört Spieler 1, nicht der KI");
...
(function ak5restart() {
  ...
  Game.newGame();
  assert.strictEqual(Game.state.awaitingStart, true,
    'Neue Runde stellt erneut auf "Los!"-Warten um');
  assert.strictEqual(Game.state.lock, false, "AK5: Neustart setzt ebenfalls kein Lock");
})();
